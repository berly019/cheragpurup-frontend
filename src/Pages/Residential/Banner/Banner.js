...
const Banner = () => {
    ...
    const {pResidentData} = useContext(DataContext);

    const bg = pResidentData?.image;
    const bg4 = {
            ...
            <div className='bg-content'>
                <Container className="p-md-5 text-center">
                    <Row xs="auto" md={2} className="align-items-center justify-content-center">
                        <Col>
                            <p className="fs-1 fw-bold">{pResidentData?.name}</p>
                            <p className="fs-2">{pResidentData?.location}</p>
                        </Col>
                    </Row>
                </Container>
            </div>
