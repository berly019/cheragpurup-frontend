...
import { Button, Col, Container, Form, Image, Row, Modal, Spinner, Alert } from 'react-bootstrap';
import { FiEdit } from 'react-icons/fi';
import PCModalEdit from './PCModalEdit/PCModalEdit';
import { DataContext } from '../../../../contexts/DataContext';

const PCommerse = () => {

    const { isLoading, pCommerceData } = useContext(DataContext);
    ...
    /*     const handleDelete = id => {
            ...
            if (proceed) {
                axios.delete(`${process.env.REACT_APP_BASE_URL}/up/pcommerce/${id}`)
                    .then((res) => {
                        console.log(res);
    ...
    return (
        <Container>
            ...
            <div key={pCommerceData._id}>
                <div>
                    <Col className="text-end" style={{ cursor: 'pointer' }}>
                        {/* <GoDiffAdded onClick={() => setModalShow(true)} />  */}
                        <Button variant="success" onClick={() => { handleShowE(); setModalEData(pCommerceData) }} size="sm"><FiEdit /> Edit</Button>
                    ...
                    <Row className="border p-3 rounded-3" md={2}>
                        <Col md={8} className="d-flex flex-column flex-md-row">
                            <Image src={pCommerceData?.image} fluid style={{ width: '216px', height: '132px' }} />
                        ...
                        <Col md={8}>
                            <Form>
                                <Form.Group as={Col} className="mb-3 d-flex flex-column flex-sm-row" controlId="formPlaintextPassword">
                                    ...
                                    <Col sm="8">
                                        <Form.Control type="text" defaultValue={pCommerceData?.name} disabled />
                                    </Col>
                                </Form.Group>
                                <Form.Group as={Col} className="mb-3 d-flex flex-column flex-sm-row" controlId="formPlaintextPassword">
                                    <Form.Label column sm="4">
                                    ...
                                    <Col sm="8">
                                        <Form.Control type="text" defaultValue={pCommerceData?.location} disabled />
                                    </Col>
                                </Form.Group>
                            </Form>
                        </Col>
            ...
            >
                ...
                <Modal.Body className="px-5">
                    <PCModalEdit data={modalEData} />
                </Modal.Body>
            </Modal>

            <Alert className="alertCss" show={showAlert} variant="light">
                Commerce Page Updated Successfully!
            </Alert>
