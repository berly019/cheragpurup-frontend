...
const CommerseTable = ({ data }) => {
  ...
  const deleteCommerce = (id) => {
    ...
    if (proceed) {
      axios
        .delete(
          `${process.env.REACT_APP_BASE_URL}/up/commerce/${id}/year/${f_year}`
        )
        .then((res) => {
          // console.log(res);
          if (res.data) {
            const remain = commerceData?.filter(
              (Commerce) => Commerce._id !== id
            );
            setCommerces(remain);
    ...
    if (num?.toString()?.length < 10) {
      ...
      if (proceed) {
        ...
        const message = `${pMainData?.title
          ...
          ?.join(
            " "
          )}${" "} ইউপি, আপনার ওয়ার্ড নংঃ ${word}, হোল্ডিং নংঃ ${holding}, পূর্বের বকেয়া করঃ ${previes_areas_tax} টাকা, ধার্যক্রিত করঃ ${assign_tax} টাকা, মোট করঃ ${total} টাকা, আদায়কৃত করঃ ${collected_tax} টাকা, বকেয়া করঃ ${areas_tax} টাকা, ধন্যবাদ।`;

        // send sms
        fetch(
          `https://api.sms.net.bd/sendsms?api_key=${apiKey}&msg=${message}&to=880${num}`
        )
        ...
        axios
          .put(
            `${process.env.REACT_APP_BASE_URL}/up/commerce/sms/${id}//year/${f_year}`,
            data
          )
          .then((res) => {
            // setSuccess(true);
      ...
      if (proceed) {
        ...
        // send sms
        fetch(
          `https://api.sms.net.bd/sendsms?api_key=${apiKey}&msg=${message}&to=88${num}`
        )
          .then((response) => response.json())
  ...
  return (
    <>
      ...
      <td
        onClick={() => handleMessageCheck(data)}
      ...
      >
        ...
        >
          ...
          <RiDeleteBinLine
            ...
            style={{ cursor: "pointer" }}
            onClick={() => deleteCommerce(modalSData?._id)}
          />
          {/* </Modal.Title> */}
        </Modal.Header>
