...
const DataContextProvider = (props) => {
  ...
  const [pageROffset, setPageROffset] = useState(
    Number(rPageNo) ? Number(rPageNo) : 0
  );
  ...
  const [pageOffset, setPageOffset] = useState(
    Number(cPageNo) ? Number(cPageNo) : 0
  );
  ...
  // !important
  const [financialYear, setFinancialYear] = useState(`২০২৪-২৫ অর্থবছর`);
  localStorage.setItem("f_year", financialYear);
  const f_year = localStorage.getItem("f_year");
  const c_year = localStorage.getItem("f_year");
  ...
  useEffect(() => {
    ...
    axios.get(`${process.env.REACT_APP_BASE_URL}/up/db_home`).then((data) => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_BASE_URL}/up/db_home/6249ebaa4c1ff21f2a664fa2`
      )
      .then((data) => {
  ...
  const updateDataTable = (id, updatedDataTable) => {
    setHomeData(
      homeData?.map((Commerce) =>
        Commerce._id === id ? updatedDataTable : Commerce
      )
    );
  };
  ...
  useEffect(() => {
    ...
    if (rWord === "all") {
      const api = `${process.env.REACT_APP_BASE_URL}/up/resident/year/${f_year}`;
      ...
      axios
        .get(
          `${process.env.REACT_APP_BASE_URL}/up/resident/word/${
            rWord ? rWord : 1
          }/year/${f_year}`
        )
        .then((data) => {
  ...
  }, [rWord, pageRNumber, rPageNo, f_year]);
  ...
  const handleRWordFilter = (id) => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_BASE_URL}/up/resident/word/${id}/year/${f_year}`
      )
      .then((data) => {
  ...
  const handleRAllFilter = () => {
    ...
    axios
      .get(`${process.env.REACT_APP_BASE_URL}/up/resident/year/${f_year}`)
      .then((data) => {
        // setFilteredData(data.data);
        setRFilteredData(data.data);
      });
  ...
  const updateResident = (id, updatedCommerce) => {
    setResidents(
      residents?.map((Resident) =>
        Resident._id === id ? updatedCommerce : Resident
      )
    );
  };
  ...
  useEffect(() => {
    ...
    if (cWord === "all") {
      axios
        .get(`${process.env.REACT_APP_BASE_URL}/up/commerce/year/${c_year}`)
        .then((data) => {
      ...
      const api = `${process.env.REACT_APP_BASE_URL}/up/commerce/word/${
        cWord ? cWord : 1
      }/year/${c_year}`;
      axios.get(api).then((data) => {
        setCommerces(data.data);
  ...
  }, [cWord, pageNumber, cPageNo, c_year]);
  ...
  const handleCWordFilter = (id) => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_BASE_URL}/up/commerce/word/${id}/year/${c_year}`
      )
      .then((data) => {
        // setFilteredData(data.data)
        setCFilteredData(data.data);
  ...
  const updateCommerce = (id, updatedCommerce) => {
    setCommerces(
      commerces?.map((Commerce) =>
        Commerce._id === id ? updatedCommerce : Commerce
      )
    );
  };
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_BASE_URL}/up/tread_license`)
      .then((data) => {
  ...
  const treadLicenseData = treadLicenses?.sort((a, b) =>
    a.name < b.name ? -1 : 1
  );

  const addTreadLicense = (
    license_no,
    institute_address,
    institute_name,
    profession_capital,
  ...
  const updateTreadLicense = (id, updatedTL) => {
    setTreadLicenses(
      treadLicenses?.map((TreadLicense) =>
        TreadLicense._id === id ? updatedTL : TreadLicense
      )
    );
  };
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_BASE_URL}/up/character_certificate`)
      .then((data) => {
  ...
  const cCharacterData = cCharacters?.sort((a, b) =>
    a.name < b.name ? -1 : 1
  );
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_BASE_URL}/up/citizen_certificate`)
      .then((data) => {
  ...
  const updateCNagorik = (id, updatedCN) => {
    setCNagoriks(
      cNagoriks?.map((TreadLicense) =>
        TreadLicense._id === id ? updatedCN : TreadLicense
      )
    );
  };
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_BASE_URL}/up/inheritance_certificate`)
      .then((data) => {
  ...
  const updateCOwarish = (id, updatedCO) => {
    setCOwarishs(
      cOwarishs?.map((Data) => (Data._id === id ? updatedCO : Data))
    );
  };
  ...
  useEffect(() => {
    axios.get(`${process.env.REACT_APP_BASE_URL}/up/wmember`).then((data) => {
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_BASE_URL}/up/entrepreneur`)
      .then((data) => {
  ...
  const addWEntrepreneur = (desi, name, phone, email, doj, image) => {
    const newData = [
      ...wEntrepreneur,
      { desi, name, phone, email, doj, image },
    ];
  ...
  const updateWEntrepreneur = (id, updatedWM) => {
    setWEntrepreneur(
      wEntrepreneur?.map((Data) => (Data._id === id ? updatedWM : Data))
    );
  };
  ...
  useEffect(() => {
    axios.get(`${process.env.REACT_APP_BASE_URL}/up/pmain`).then((data) => {
      setPMain(data.data);
      setIsLoading(true);
    });
  }, []);
  ...
  useEffect(() => {
    axios.get(`${process.env.REACT_APP_BASE_URL}/up/pbimage`).then((data) => {
  ...
  useEffect(() => {
    axios.get(`${process.env.REACT_APP_BASE_URL}/up/intro`).then((data) => {
      setIntro(data.data);
      setIsLoading(true);
    });
  }, []);
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_BASE_URL}/up/data_table`)
      .then((data) => {
  ...
  useEffect(() => {
    axios.get(`${process.env.REACT_APP_BASE_URL}/up/president`).then((data) => {
  ...
  const updatePResident = (id, updatedPC) => {
    setPResident(
      pResident?.map((Data) => (Data._id === id ? updatedPC : Data))
    );
  };
  ...
  useEffect(() => {
    axios.get(`${process.env.REACT_APP_BASE_URL}/up/pcommerce`).then((data) => {
  ...
  const updatePCommerce = (id, updatedPC) => {
    setPCommerce(
      pCommerce?.map((Data) => (Data._id === id ? updatedPC : Data))
    );
  };
  ...
  useEffect(() => {
    axios.get(`${process.env.REACT_APP_BASE_URL}/up/pcontact`).then((data) => {
  ...
  // notice
  useEffect(() => {
    axios.get(`${process.env.REACT_APP_BASE_URL}/up/notice`).then((data) => {
      setNotice(data.data);
      setIsLoading(true);
    });
  }, []);
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_BASE_URL}/up/run_notice`)
      .then((data) => {
  ...
  const updateRunNotice = (id, updatedPC) => {
    setRunNotice(
      runNotice?.map((Data) => (Data._id === id ? updatedPC : Data))
    );
  };
